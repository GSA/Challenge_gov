defmodule Web.AgencyInvittationController do
  use Web, :controller
  ...
  def index(conn, params = %{"team_id" => team_id}) do
    ...
    with {:ok, team} <- Agencies.get(team_id) do
      conn
      |> assign(:team, team)
  ...
  def create(conn, %{"team_id" => team_id, "user_id" => user_id}) do
    ...
    with {:ok, team} <- Agencies.get(team_id),
         true <- Agencies.member?(team, user),
         {:ok, invitee} <- Accounts.public_get(user_id),
         {:ok, _member} <- Agencies.invite_member(team, user, invitee) do
      conn
  ...
  def accept(conn, %{"team_id" => team_id}) do
    ...
    with {:ok, team} <- Agencies.get(team_id),
         {:ok, _member} <- Agencies.accept_invite(team, user) do
      conn
  ...
  def reject(conn, %{"team_id" => team_id}) do
    ...
    with {:ok, team} <- Agencies.get(team_id),
         {:ok, _member} <- Agencies.reject_invite(team, user) do
      conn
