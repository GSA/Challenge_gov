defmodule Web.UserView do
  ...
  def certification_info(certification, %{status: status}) do
    ...
    if after? || status == "deactivated" do
      ~E"""
        <span><b>Decertified On:</b> <%= one_year_from_certification.month %>/<%= one_year_from_certification.day %>/<%= one_year_from_certification.year %></span>
      """
    else
      ~E"""
        <span><b>Due On:</b> <%= certification.expires_at.month %>/<%= certification.expires_at.day %>/<%= certification.expires_at.year %></span>
      """
    end
  end
