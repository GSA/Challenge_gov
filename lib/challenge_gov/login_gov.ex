defmodule ChallengeGov.LoginGov do
  ...
  def process_request_options(options) do
    [
      {:proxy,
       ~c"http://#{Application.get_env(:challenge_gov, :http_proxy_user)}:#{Application.get_env(:challenge_gov, :http_proxy_pass)}@#{Application.get_env(:challenge_gov, :http_proxy)}:61443"}
    ]
  end
