    ...
    if (challenge.logo) {
      return (
        <div className="image_wrapper">
          <img src={challenge.logo} alt={`Logo for ${challenge.agency_name}`} className="w-100" />
        </div>
      )
    }
  ...
  return (
    challenge ? (
      <div key={challenge.id} className="challenge-tile card">
        <a href={challengeTileUrl(challenge, preview)} target={challenge.external_url ? "_blank" : ""}>
          {renderTileLogo()}
          <div className="challenge-tile__text-wrapper">
            <div role="heading" aria-level="2">
              <h2 className="challenge-tile__title test" style={{ textAlign: 'left', paddingLeft: '20px', paddingTop: '20px', lineHeight: '30px' }}>
                {truncateString(challenge.title, 90)}
              </h2>
            </div>
            <p className="challenge-tile__agency-name">{truncateString(challenge.agency_name, 90)}</p>
            <p className="challenge-tile__tagline">{truncateString(challenge.tagline, 90)}</p>
